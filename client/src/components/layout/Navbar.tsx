...
import { Link, useNavigate } from "react-router-dom";
import { Menu, User, Settings, HelpCircle, LogOut, Crown } from "lucide-react";
import image from "@/assets/avatars/avatar-1.jpg";
...
const Navbar: React.FC<NavbarProps> = ({ onMobileMenuToggle }) => {
  ...
  return (
    <div className="w-full py-3 md:py-4 px-8 md:px-6 border-b border-border select-none bg-card/50 backdrop-blur-md sticky top-0 z-40">
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        <Button
          variant="ghost"
          size="icon"
        ...
        <Link
          to="/"
          className="text-xl md:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-primary via-accent to-primary bg-clip-text text-transparent hover:opacity-80 transition-opacity"
        >
          Social Media
        </Link>
